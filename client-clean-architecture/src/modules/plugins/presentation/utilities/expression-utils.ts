...
export function getAvailableExpressions(
    ...
    for (const ancestor of ancestors) {
        ...
        for (const [key, info] of Object.entries(schema) as [string, { type: string; description?: string; children?: NodeOutputSchema }][]) {
            expressions.push({
                nodeId: ancestor.id,
                nodeName,
                nodeType,
            ...
            if (info.children) {
                for (const [childKey, childInfo] of Object.entries(info.children!) as [string, { type: string; description?: string }][]) {
                    expressions.push({
                        nodeId: ancestor.id,
                        nodeName,
                        nodeType,
