...
import { formatSize } from '@/modules/canvas/presentation/utilities/scene-utils';
import { formatDistanceToNow } from 'date-fns';
import { getChatUseCases } from '@/modules/chat/application/registry';
import Tooltip from '@/shared/presentation/components/atoms/common/Tooltip';
import '@/modules/chat/presentation/components/molecules/SharedFilesList/SharedFilesList.css';
...
const SharedFilesList = ({ messages, currentChatId }: SharedFilesListProps) => {
    const { getChatFilePreviewUseCase } = getChatUseCases();
    ...
    useEffect(() => {
        ...
        const run = async () => {
            for (const m of fileMessages) {
                ...
                try {
                    const p = await getChatFilePreviewUseCase.execute(currentChatId, m._id);
    ...
    }, [fileMessages.map(f => f._id).join(','), currentChatId, getChatFilePreviewUseCase]);
