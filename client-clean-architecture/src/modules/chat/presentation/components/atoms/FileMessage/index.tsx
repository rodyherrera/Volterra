...
import '@/modules/chat/presentation/components/atoms/FileMessage/FileMessage.css';
...
const FileMessage: React.FC<FileMessageProps> = ({ msg, currentChatId }: FileMessageProps) => {
    ...
    useEffect(() => {
        ...
        const run = async() => {
            try{
                const res = await getChatFilePreviewUseCase.execute(currentChatId, msg._id);
                if(!cancelled){
    ...
    }, [isImage, currentChatId, msg._id, getChatFilePreviewUseCase]);
