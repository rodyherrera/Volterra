...
import '@/modules/clusters/presentation/components/molecules/DiskOperations/DiskOperations.css';
...
export function DiskOperations({ metrics }: DiskOperationsProps) {
  ...
  */
  ...
  const isLoading = !metrics // || !isHistoryLoaded || !metrics?.diskOperations || history.length === 0;
    ...
    <div className="d-flex column flex-1 disk-ops-chart">
      ...
      <div className="d-flex content-center gap-1 disk-ops-legend f-shrink-0">
        <div className="d-flex items-center gap-0375 disk-ops-legend-item font-size-1 color-muted-foreground">
          ...
          <span className="disk-ops-legend-label font-size-1 color-secondary">Read(MB/s)</span>
        </div>
        <div className="d-flex items-center gap-0375 disk-ops-legend-item font-size-1 color-muted-foreground">
          <span className="disk-ops-legend-dot" style={{ backgroundColor: '#30D158' }}></span>
          <span className="disk-ops-legend-label font-size-1 color-secondary">Write(MB/s)</span>
        </div>
        <div className="d-flex items-center gap-0375 disk-ops-legend-item font-size-1 color-muted-foreground">
          <span className="disk-ops-legend-dot" style={{ backgroundColor: '#FF9F0A' }}></span>
          <span className="disk-ops-legend-label font-size-1 color-secondary">IOPS(x10)</span>
        </div>
      </div>
    </div>
  );
