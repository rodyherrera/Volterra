...
export const createCameraSlice: StateCreator<any, [], [], CameraSlice> = (set, get) => ({
    camera: {
        ...
        setType: (type) => set((state: any) => ({
            camera: { ...state.camera, type }
        })),

        setPosition: (position) => set((state: any) => ({
            camera: { ...state.camera, position }
        })),

        setUp: (up) => set((state: any) => ({
            camera: { ...state.camera, up }
        })),

        setPerspective: (partial) => set((state: any) => ({
            camera: {
                ...state.camera,
        ...
        setOrthographic: (partial) => set((state: any) => ({
            camera: {
                ...state.camera,
