	...
	useEffect(() => {
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (e.ctrlKey && e.altKey && e.key.toLowerCase() === 'z') {
				e.preventDefault();
				const modelBounds = (activeModel as any)?.modelBounds;
				if (orbitControlsRef.current && modelBounds) {
					const { position: pos, target, up } = calculateCameraPositionForFace(
						modelBounds.center,
						modelBounds.size,
						'px', // Closest to ZY plane
						orbitControlsRef.current.object.fov,
						orbitControlsRef.current.object.aspect
					);
					orbitControlsRef.current.object.position.set(pos.x, pos.y, pos.z);
					orbitControlsRef.current.target.set(target.x, target.y, target.z);
					orbitControlsRef.current.object.up.set(up.x, up.y, up.z);
					orbitControlsRef.current.object.lookAt(target.x, target.y, target.z);
					orbitControlsRef.current.update();
				}
			}
		};
	...
	return (
		<div style={{ width: '100%', height: '100%' }}>
			...
			>
				...
				<ThreeColor attach="background" args={[threeBackgroundColor]} />
				<Preload all />
