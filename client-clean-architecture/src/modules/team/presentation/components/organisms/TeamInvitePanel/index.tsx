...
import { IoClose, IoCheckmark, IoBook } from 'react-icons/io5';
...
import type { TeamInvitation } from '@/modules/team-invitation/domain/entities';
import { teamInvitationRepository } from '@/modules/team-invitation/infrastructure/repositories/TeamInvitationRepository';
import '@/modules/team/presentation/components/organisms/TeamInvitePanel/TeamInvitePanel.css';
...
const TeamInvitePanel: React.FC<TeamInvitePanelProps> = ({
    ...
    const [generalAccess] = useState<'Can edit' | 'Can view' | 'Restricted'>('Restricted');
    ...
    const handleAddMember = async (e: React.KeyboardEvent<HTMLInputElement> | React.MouseEvent<HTMLButtonElement>) => {
        ...
        try {
            await teamInvitationRepository.send(email.trim(), 'Can view');

            const updatedInvitations = await teamInvitationRepository.getPending();
    ...
    const handleCancelInvitation = async (invitationId: string, inviteEmail: string) => {
        try {
            await teamInvitationRepository.cancel(invitationId);
            setInvitations(invitations.filter(inv => inv._id !== invitationId));
            showSuccess(`Invitation to ${inviteEmail} cancelled`);
        } catch (err) {
    ...
    const getAvatarColor = (inviteEmail: string) => {
        const colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A', '#98D8C8'];
        const hash = inviteEmail.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
        return colors[hash % colors.length];
    };

    const getInitials = (inviteEmail: string) => {
        return inviteEmail.split('@')[0].charAt(0).toUpperCase();
    };
