import { Response } from "express";
import { injectable } from "tsyringe";
import BaseResponse from "@/src/shared/infrastructure/http/BaseResponse";
import { AuthenticatedRequest } from "@/src/shared/infrastructure/http/middleware/authentication";
import UpdateTeamMemberByIdUseCase from "@/src/modules/team/application/use-cases/team-member/UpdateTeamMemberByIdUseCase";

@injectable()
export default class UpdateTeamMemberByIdController{
    constructor(
        private useCase: UpdateTeamMemberByIdUseCase
    ){}

    async handle(req: AuthenticatedRequest, res: Response): Promise<void>{
        const { teamMemberId } = req.params;
        const { roleId } = req.body;
        const result = await this.useCase.execute({ teamMemberId, roleId });
        if(!result.success){
            BaseResponse.error(res, result.error.message, result.error.statusCode);
            return;
        }
        BaseResponse.success(res, result.value, 200);
    }
};