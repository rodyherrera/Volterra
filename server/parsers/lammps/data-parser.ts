...
export default class LammpsDataParser {
    ...
    public parse(filePath: string, options: ParseOptions = {}): ParseResult {
        ...
        // Augment metadata with simulation cell. 
        // For accurate Triclinic support, we SHOULD read the header, but since we are inside `parse`, 
        // we can try to rely on parseMetadataOnly if we want, OR just assume orthogonal from min/max for now
        // if headers are not cheap to read.
        // However, robust correctness suggests we should try to get the cell geometry.

        let metadataWithCell = result.metadata as FrameMetadata;

        // Try to read header to get accurate simulation cell (especially for triclinic)
        try {
            // Read first 1KB which should contain headers
            ...
            if (metaOnly && metaOnly.simulationCell) {
                ...
                // Fallback to orthogonal from min/max
    ...
    public canParse(headerLines: string[]): boolean {
        ...
        // Relaxed check: "atoms" count and at least one box bound def
    ...
    /**
     * Parse only metadata from header lines (Pure JS).
     * Extracts simulation cell geometry from LAMMPS data file header.
     */
    public parseMetadataOnly(headerLines: string[]): FrameMetadata {
        ...
        let simulationCell: any = {
            ...
            geometry: {
                ...
                cell_origin: [0, 0, 0],
                periodic_boundary_conditions: { x: true, y: true, z: true } // Data files usually imply PBC
            }
        };

        const content = headerLines.join('\n');

        // Parse Timestep (if available in comment or standard header)
        ...
        // Parse Box Bounds
        // Regex to capture scientific notation floats
