...
int StructureAnalysis::findCommonNeighbors(
...
int StructureAnalysis::findNeighborBonds(
	const NeighborBondArray& neighborArray, 
	unsigned int commonNeighbors,
	int numNeighbors, 
...
static int getAdjacentBonds(
	unsigned int atom, 
	StructureAnalysis::CNAPairBond* bondsToProcess, 
	int& numBonds, 
    ...
    for(int b = numBonds - 1; b >= 0; --b){
        if(atom & bondsToProcess[b]){
            ...
            memmove(&bondsToProcess[b], &bondsToProcess[b+1], sizeof(StructureAnalysis::CNAPairBond) * (numBonds - b - 1));
            --numBonds;
        }
    }
...
int StructureAnalysis::calcMaxChainLength(CNAPairBond* neighborBonds, int numBonds){
...
CoordinationStructure StructureAnalysis::_coordinationStructures[NUM_COORD_TYPES];

/// Contains the known lattice types.
LatticeStructure StructureAnalysis::_latticeStructures[NUM_LATTICE_TYPES];
	...
	_identifyPlanarDefects(identifyPlanarDefects)
{
	...
	_neighborLists = std::shared_ptr<ParticleProperty>(new ParticleProperty(positions->size(), DataType::Int,
			latticeStructure(inputCrystalType).maxNeighbors, 0, "Neighbors", false));
...
bool StructureAnalysis::identifyStructures(){
	...
	ParallelSystem::parallelFor(positions()->size(), [this, &neighFinder, &threadMaxDistances](size_t index){
		double localMaxDistance = determineLocalStructure(neighFinder, index);
...
double StructureAnalysis::determineLocalStructure(NearestNeighborFinder& neighList, size_t particleIndex){
	assert(_structureTypes->getInt(particleIndex) == COORD_OTHER);

	// Skip atoms that are not included in the analysis.
	if(_particleSelection && _particleSelection->getInt(particleIndex) == 0) return 0.0;
	...
	if(_inputCrystalType == LATTICE_FCC || _inputCrystalType == LATTICE_HCP){
		...
		for(int ni = 0; ni < 4; ni++){
			cnaSignatures[ni] = 0;
			unsigned int commonNeighbors;
			if(findCommonNeighbors(neighborArray, ni, commonNeighbors, nn) != 3) return 0.0;
		}
