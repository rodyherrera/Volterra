...
void DislocationTracing::generateOutputMesh(){
	...
	for(auto facet = facets.begin(); facet != facets.end(); ++facet){
		...
		for(int v = 0; v < 3; v++){
			...
			if(node1->testFlag(ATOM_DISCLINATION_BORDER)) isDisclinationBarrier = true;
		}

		// Create new output facet.
		OutputFacet* newfacet = outputMesh.createFacet(outputEdges);

		if(isDisclinationBarrier) newfacet->setFlag(OUTPUT_FACET_IS_DISCLINATION_BARRIER);
	}
	...
	for(auto circuit_iter = danglingCircuits.begin(); circuit_iter != danglingCircuits.end(); ++circuit_iter){
		...
		for(auto e = circuit->primarySegmentCap.begin(); e != circuit->primarySegmentCap.end(); ++e){
			MeshEdge* meshEdge = (*e);
			MeshNode* node1 = meshEdge->node1;
			MeshNode* node2 = meshEdge->node2();
...
void InterfaceMesh::smoothOutputSurface(int smoothingLevel){
	if(smoothingLevel < 0) {
		LOG_ERROR() << "Smoothing level must be non-negative.";
		return;
	}
...
void InterfaceMesh::finishOutputSurface(bool createCapSurface){
	...
	if(createCapSurface) outputMesh.wrapMesh(*this, &outputMeshCap);
	else outputMesh.wrapMesh(*this);
}
...
void DislocationTracing::smoothDislocationSegments(int smoothingLevel, int coarseningLevel){
	...
	if(coarseningLevel > 0){
		...
		for(int segmentIndex = 0; segmentIndex < segments.size(); segmentIndex++) {
			...
			int counter = 0;
			for(auto p = line.begin() + 1; p != line.end() - 1;){
...
void DislocationTracing::wrapDislocationSegments(){
	// Nothing to do if there are no periodic boundaries.
...
void DislocationTracing::clipDislocationLines(const Point3& clipOrigin, const Matrix3& clipCell){
	...
	for(size_t segmentIndex = 0; segmentIndex < oldSegmentCount; segmentIndex++){
		...
		for(auto p2_in = p1_in + 1; p2_in != line.end(); ++p2_in){
