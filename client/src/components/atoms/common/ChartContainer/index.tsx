    ...
    <Container className="d-flex h-max column p-1-5 chart-container sm:p-1">
      <Container className="d-flex content-between mb-1-5 sm:column sm:gap-1">
        <Container className="d-flex items-center gap-075">
          ...
          <Title className='font-size-3 chart-title font-weight-6'>{title}</Title>
        </Container>
        {stats && (
          <Container className="d-flex gap-1-5 flex-wrap sm:w-max sm:gap-1">
            {stats.map((stat) => (
              <Container key={stat.label} className="d-flex column gap-025">
                <span className="chart-stat-label font-size-1 color-muted-foreground">{stat.label}</span>
                {statsLoading ? (
                  <Skeleton variant="text" width={60} height={18} />
                ) : (
                  <span className="chart-stat-value font-size-3">{stat.value}</span>
                )}
              </Container>
            ))}
          </Container>
